<template>
    <div class="box">
        ...
        <div>
            <ul class="ul1">
                <li v-for="(item, index) in tet1" :key="index">
        ...
        <div class="rightdata">
            <div class="footer">
                ...
                <input class="input" type="Text1" placeholder="添加任务" v-model="inputValue" @focus="startAnimation"
                    @blur="resetProgress" @keyup.enter="handleEnterKey" />
                <div class="jindu">
                    <el-progress :percentage="progressValue" color="#1089ff" :show-Text1="false"
...
export default {
    ...
    mounted() {
        // 从本地存储中获取数据
        const storedItems = localStorage.getItem("Text1");
        if (storedItems) {
            this.tet1 = JSON.parse(storedItems);
        }
    },
    ...
    methods: {
        del(index) {
            if (confirm("确定要删除吗？")) {
                this.tet1.splice(index, 1)
            } else {
                return
            }
            localStorage.setItem("Text1", JSON.stringify(this.tet1));
        },
        startAnimation() {
